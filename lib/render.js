...
exports.template = function (url, html, time, component) {
    if (!time) {
        time = typeof (html) == 'number' ? html : (time || config.TemplateDefaultTime);
    }
    var componentStr = component ? '-component' : '';
    url = html ? url = path.resolve(config.templateUrl, url) : url;
    var isVirtual = html ? true : false
    ...
    if (config.templateMap[url + componentStr]) {
        if (config.isSaveFile) {
            ...
            return config.templateMap[url + componentStr].str;
        }
    }
    try {
        if (!html) {
        ...
        var str = 'var str = "'+(component?'':'<!DOCTYPE html>')+'";' + EachJsonTree(treeList, importList, false, component) + ";return str;";
        if (config.isSaveTemplate || isVirtual) {
            if (config.isSaveFile) {
...
exports.refreshTemplate = function (url, html, time, component) {
    ...
    try {
        if (!html) {
            if (!path.isAbsolute(url)) {
                url = path.resolve(config.templateUrl, url)
            }
            html = fs.readFileSync(url).toString();
        }
        var _a = HtmltoparseDom(html, true), treeList = _a.treeList, importList = _a.importList;
...
exports.expressRender = function (filePath, options, callback) {
    ...
    return callback(null, exports.render(filePath, options));
};
/**
 * 修改render默认配置
 * @param name
...
exports.updateDirective = function(d){
    for(let key in d){
        directive[key] = d[key];
    }
